...
static pthread_mutex_t residuum_mutex = PTHREAD_MUTEX_INITIALIZER;
/* ************************************************************************ */
/* thread_calc: actual math happens here                                    */
/* ************************************************************************ */
static
void*
thread_calc(void* params)
{
    int i, j;
    ...
    for (i = args->slices[args->thread_id * 2] + 1; i < args->slices[args->thread_id * 2 + 1]; i++)
    {
	    ...
	    //printf("in zeile: %d, spalte: %d", i, j);
            ...
            {
                star += (0.25 * TWO_PI_SQUARE * args->arguments->h * args->arguments->h) * sin((PI * args->arguments->h) * (double)i) * sin((PI * args->arguments->h) * (double)j);
            }
...
calculate (struct calculation_arguments const* arguments, struct calculation_results *results, struct options const* options)
{
    ...
    double x = 0;
    double y = 0;
	double* residuum = &x;                           /* residuum of current iteration                  */
	double* maxresiduum = &y;                        /* maximum residuum value of a slave in iteration */
    int nthreads = options->number;
	...
	while (term_iteration > 0)
	{
		*maxresiduum = 0;
        ...
        for(i = 0; i < nthreads; ++i)
        {
            ...
            pthread_create(&threads[i], NULL, thread_calc, (void *)args);
        }
