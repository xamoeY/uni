...
calculate (struct calculation_arguments const* arguments, struct calculation_results *results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		if(rank > 0)
			MPI_Recv(Matrix_Out[0], N_global + 1, MPI_DOUBLE, rank - 1, rank - 1 + results->stat_iteration, 
			MPI_COMM_WORLD, NULL);

		if(results->stat_iteration > 0)	
			if(rank != nproc - 1)
				MPI_Recv(Matrix_Out[N], N_global + 1, MPI_DOUBLE, rank + 1, rank + 1 + results->stat_iteration - 1,
				MPI_COMM_WORLD, NULL);
		...
		if(term_iteration > 1)
			if(rank > 0)
				MPI_Send(Matrix_Out[1], N_global + 1, MPI_DOUBLE, rank - 1, rank + results->stat_iteration, 
				MPI_COMM_WORLD);

		if(rank != nproc - 1)
			MPI_Send(Matrix_Out[N - 1], N_global + 1, MPI_DOUBLE, rank + 1, rank + results->stat_iteration,		
			MPI_COMM_WORLD);
