...
converge to an image of acceptable quality. Every ray has a certain maximum depth \(D\). So far,
this yields the following formula:

\[ O_w \times O_h \times S \times D \]

This assumes, however, that the scene lookup time for every ray is \(1\) which is only
the case for scenes with a single triangle in them.

With a suitable spatial data structure, the scene lookup time per ray is in \(O(\log n)\).
